...
/**
 * What the chapter has already put on the page, in the forms the next scene actually needs: the
 * journal of what the written scenes established, quoted from them, and the last words, so the voice
 * continues. Pasting every finished scene in full made the prose already written 43.5% of a
 * 78,000-character prompt while the scene to write was 0.7% of it, and a live chapter came back as its second scene written four times:
...
function alreadyOnThePage(chapter: ChapterRecord, sceneIndex: number): string {
  ...
  return (journal.length ? `\nWHERE THIS CHAPTER STANDS — taken from the scenes already written, each note with the line that establishes it. This is a draft record of a chapter still being written, not accepted canon; where it and the prose below disagree, the prose is what happened:\n${JSON.stringify(journal)}` : '')
