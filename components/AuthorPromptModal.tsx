...
const AuthorPromptModal: React.FC<AuthorPromptModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-start justify-center z-50 p-4 pt-20 overflow-y-auto">
      <div className="bg-zinc-900 rounded-sm shadow-2xl max-w-md w-full border border-zinc-800">
        <form onSubmit={handleSubmit}>
          <div className="p-6">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-zinc-950 border border-zinc-700 rounded-sm text-zinc-100 placeholder-zinc-500 text-sm focus:outline-none focus:ring-1 focus:ring-zinc-400 focus:border-zinc-400"
              autoFocus
            />
          </div>
          <div className="flex justify-end gap-3 p-4 bg-zinc-950/60 border-t border-zinc-800/80 rounded-b-sm">
            <Button
              type="button"
              onClick={onCancel}
              variant="secondary"
